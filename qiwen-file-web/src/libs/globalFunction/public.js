...
const fileFunction = {
	 ...
	 */
	getMinImgStream(row) {
		return `${config.baseContext}/publictransfer/preview?userFileId=${
			row.userFileId
	...
	getDownloadFilePath(row) {
		return `${config.baseContext}/publictransfer/downloadfile?userFileId=${
			row.userFileId
		}&shareBatchNum=${
			row.shareBatchNum == null ? '' : row.shareBatchNum
		}&extractionCode=${row.extractionCode == null ? '' : row.extractionCode}`
	...
	handleImgPreview(currentIndex, imgInfo = {}, imgInfoList = []) {
		...
		Vue.prototype.$openBox2.imgPreview({ imgList, defaultIndex })
	},
	/**
	 * 视频预览
	...
	handleVideoPreview(currentIndex, videoInfo = {}, videoInfoList = []) {
		...
		Vue.prototype.$openBox2.videoPreview({ videoList, defaultIndex })
	},
	/**
	 * 音频预览
	...
	handleAudioPreview(currentIndex, audioInfo = {}, audioInfoList = []) {
		...
		Vue.prototype.$openBox2.audioPreview({ audioList, defaultIndex })
	},
	/**
	 * 文件预览
	...
	handleFileNameClick(row, currentIndex, fileList = []) {
		...
		// 若是文件，则进行相应的预览
			...
			if (
				...
				Vue.prototype.$openBox2.codePreview({ fileInfo: row, isEdit: false })
				return false
			}
			//  若当前点击项是 markdown 文档
			if (markdownFileType.includes(row.extendName.toLowerCase())) {
				Vue.prototype.$openBox2.markdownPreview({
					fileInfo: row,
					editable: false
				})
				return false
