...
export default {
    ...
    ...mapState({
      showUploadMask: state => state.uploadFile.showUploadMask
		...
		batchDownloadLink() {
			...
			}/publictransfer/batchDownloadFile?userFileIds=${this.selectedFiles
	...
	methods: {
		...
		handleUploadFileBtnClick(uploadWay) {
			this.$openDialog2.authWeChat({}).then((res) => {
				switch (res) {
					case 'confirm': {
		...
		handleFileDisplayModelChange(label) {
			...
			// 关闭右键菜单事件
			this.$openBox2.contextMenu.close()
